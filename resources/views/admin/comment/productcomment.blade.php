...
@php
   ...
   $countSeparatePendingComment = countSeparatePendingComment();
@endphp
<head>
	<title>D3 Teknik Informatika - Event Comment</title>
</head>
<style>
    .navbar {
        margin-bottom: 0;
	...
	<nav class="navbar navbar-default">
		<div class="container-fluid">
			<ul class="nav navbar-nav">
				<li class="{{ Request::is('d3ti-admin/all_comment/post_comment*') ? 'active' : '' }}"><a href="{{ url('d3ti-admin/all_comment/post_comment') }}">Post Comment ({{ $countAllComment['post'] }})<div style="color: #0088CC">({{ $countSeparatePendingComment['post'] }} Pending)</div></a></li>
				<li class="{{ Request::is('d3ti-admin/all_comment/event_comment*') ? 'active' : '' }}"><a href="{{ url('d3ti-admin/all_comment/event_comment') }}">Event Comment ({{ $countAllComment['event'] }})<div style="color: #0088CC">({{ $countSeparatePendingComment['event'] }} Pending)</div></a></li>
				<li class="{{ Request::is('d3ti-admin/all_comment/product_comment*') ? 'active' : '' }}"><a href="{{ url('d3ti-admin/all_comment/product_comment') }}">Product Comment ({{ $countAllComment['product'] }})<div style="color: #0088CC">({{ $countSeparatePendingComment['product'] }} Pending)</div></a></li>
			</ul>
		</div>
	</nav>
	...
	<section class="panel">
		<div class="panel-body">
			<table class="table table-bordered table-striped mb-none" id="datatable-default">
				...
				<tbody>
                    @foreach($data_product_comment as $data)
	                    @php
	                    	$status = $data->product_comment_status;

			                $dateString = $data->created_at;
			                $date = new DateTime($dateString);
			                $formattedDate = $date->format('Y-m-d H:i:s');
	                    	$formApproveId = "approve-form-" . $data->product_comment_id;
	                    	$formRejectId = "reject-form-" . $data->product_comment_id;
	                    @endphp
					<tr>
						<td>{{ $data->product_comment_id }}</td>
						<td>{{ $data->product_comment_value }}
							<br/>
							@if ($status == "Pending")
							    ...
							    <form id="{{ $formApproveId }}" method="post" action="{{ url ('d3ti-admin/all_comment/product_comment/approve') }}/{{ $data->product_comment_id }}">
							        @csrf
							        @method('PUT')
							    </form>

							    <form id="{{ $formRejectId }}" method="post" action="{{ url ('d3ti-admin/all_comment/product_comment/reject') }}/{{ $data->product_comment_id }}">
							        @csrf
							        @method('PUT')
							    </form>
							@else
						...
						<td>{{ $data->product_comment_name }}</td>
						<td>{{ $data->product_comment_email }}</td>
						<td>{{ $data->product_comment_status }}</td>
						<td><a href="https://whatismyipaddress.com/ip/{{ $data->product_comment_ip }}">{{ $data->product_comment_ip }}</a></td>
						<td>{{ $formattedDate }}</td>
					</tr>
					@endforeach
				</tbody>
